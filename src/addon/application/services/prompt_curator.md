...
- the note's field notation is wrong: a line break stored as a literal newline (Anki's editor renders fields as HTML, so it folds to a space and the line structure is lost), a run of raw spaces or a tab (HTML collapses it, so indentation reads wrong wherever the field is shown as HTML), or a raw `<`, `>`, or `&` anywhere in the field (Anki reads a raw `<` as an HTML tag before the markdown renderer runs, so `<rev>` — and the text the tag swallows — disappears from the card while the note gains a stray closing tag). Any of these is a defect even if everything else about the note is fine: propose an edit that writes every break as `<br>`, every extra space as `&nbsp;`, and the characters as entities — `&lt;`, `&gt;`, `&amp;` — in prose, code, and math alike, and changes nothing else. Never turn a stored entity back into its raw character.
...
- Preserve what works: the user's voice and formatting conventions (Markdown formatting, math delimiters, code fences), their examples, images, and personal anchors. Keep existing sources and date stamps; date-stamp claims that age ("as of 2025"). Never invent examples or sources. Front and back are Markdown fields rendered by the Better Markdown addon, with HTML-significant characters stored as entities (`&lt;`, `&gt;`, `&amp;`); math is written in \(...\) / \[...\] form and code in fences or backticks, as laid out under 'Ideal format for each note'.
...
Fields are Markdown, but they are stored in Anki's HTML flavour — what Anki's own editor produces and what the addon normalizes back before parsing. A line break is `<br>` everywhere (prose, code fences, and math alike) and a blank line is `<br><br>`; indentation and runs of spaces are `&nbsp;`; and `<`, `>`, `&` are entities — `&lt;`, `&gt;`, `&amp;` (escape `&` first). The addon turns the `<br>` back into newlines, the `&nbsp;` back into spaces, and decodes the entities, so the card renders exactly as the markdown reads — and the field also reads correctly in Anki's editor. A literal newline or a run of raw spaces collapses wherever the field is shown as HTML; a raw `<` is read as an HTML tag before any renderer runs, so `<rev>` and everything the tag swallows vanish from the card. Never unescape a stored entity, and never change the encoding of content you are not deliberately rewriting.
