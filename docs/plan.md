...
| Q4 2026 | Note fields on the Better Markdown note types store Anki's HTML flavour: every line break is `<br>` (a blank line `<br><br>`), every extra space is `&nbsp;`, `<`/`>`/`&` are entities, and code is fenced with a language tag | The addon reads each field's `innerHTML` and normalizes `<br>`/`<div>` back to newlines, `&nbsp;` to spaces, and entities to characters before parsing with ReactMarkdown + KaTeX + react-syntax-highlighter. A raw `<` is parsed as an HTML tag by Anki before any renderer runs (a fence holding `<rev>` loses its placeholder and gains a stray closing tag), and raw newlines/runs of spaces collapse in Anki's HTML editor so the stored code reads wrong there — the HTML flavour is the only form that is both render-correct and editor-readable |
